...
def RemoveInconsistentValues(queens,i,j):
 ...
 removed = 0
 absValue = abs(i-j)
 print 'Possible values for {0} : {1}'.format(i,possible_values[i])
 print 'Possible values for {0} : {1}'.format(j,possible_values[j])
 for x in possible_values[i]:
  print 'x = ',x
		...
		#print(queens)
			...
			if not under_attack(i,queens):
				...
				possible_values[len(queens)] = [i]
                                #print 'Possible values inside rsolve : ',possible_values
				constraints = []
